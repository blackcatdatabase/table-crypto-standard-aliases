...
namespace BlackCat\Database\Packages\CryptoStandardAliases\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\CryptoStandardAliases\Dto\CryptoStandardAliaseDto;
use BlackCat\Database\Packages\CryptoStandardAliases\Mapper\CryptoStandardAliaseDtoMapper;
 ...
 */
final class CryptoStandardAliasesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CryptoStandardAliaseRepository $cryptoStandardAliaseRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'crypto_standard_aliases' => $this->cryptoStandardAliaseRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\CryptoStandardAliases\ModuleException("Record {$idStr} not found for locking.");
            }
