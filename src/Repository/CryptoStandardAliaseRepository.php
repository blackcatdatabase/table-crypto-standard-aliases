...
namespace BlackCat\Database\Packages\CryptoStandardAliases\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\CryptoStandardAliases\Definitions;
use BlackCat\Database\Packages\CryptoStandardAliases\Criteria;
use BlackCat\Database\Packages\CryptoStandardAliases\Dto\CryptoStandardAliaseDto as Dto;
use BlackCat\Database\Packages\CryptoStandardAliases\Mapper\CryptoStandardAliaseDtoMapper as RowMapper;
...
final class CryptoStandardAliaseRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\CryptoStandardAliases\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'alias' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\CryptoStandardAliases\Definitions::table());
            return $bulk->upsertMany($rows, [ 'alias' ], [ 'algo_id', 'notes' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CryptoStandardAliases\Dto\CryptoStandardAliaseDto|null */
    public function getByAlias(string $alias, bool $asDto = false): array|\BlackCat\Database\Packages\CryptoStandardAliases\Dto\CryptoStandardAliaseDto|null {
        $row = $this->getByUnique([ 'alias' => $alias ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\CryptoStandardAliases\Mapper\CryptoStandardAliaseDtoMapper::fromRow($row);
    }
    public function existsByAlias(string $alias): bool {
        $where = 't.' . Ident::q($this->db, 'alias') . ' = :uniq_alias';
        return $this->exists($where, [ 'uniq_alias' => $alias ]);
    }
    /** @return int|string|null */
    public function getIdByAlias(string $alias) {
        $row = $this->getByAlias($alias);
        return $row ? ($row['alias'] ?? null) : null;
    }
