...
use BlackCat\Database\Packages\CryptoStandardAliases\Repository\CryptoStandardAliaseRepositoryInterface;
...
class CryptoStandardAliaseRepository implements CryptoStandardAliaseRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'alias' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\CryptoStandardAliases\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'algo_id', 'notes' ]);
          return count($rows);
        }
