...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'alias', 'algo_id', 'notes', 'created_at' ];
    }
    ...
    {
        $x = [ 'alias', 'algo_id', 'notes', 'created_at' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\CryptoStandardAliases\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\CryptoStandardAliases\Definitions::softDeleteColumn();
    ...
    public function byId(int|string $id): self {
        return $this->where('t.alias = :cid', ['cid' => $id]);
    }
    public function byIds(array $ids): self {
        if (!$ids) return $this->where('1=0');
        return $this->whereIn('t.alias', array_values($ids));
    }
